...
export function validateAgentId(id) {
    ...
    if (id === 'main')
        return { ok: false, error: '"main" is reserved for the primary bot' };
    if (id.startsWith('_'))
        return { ok: false, error: 'Agent IDs starting with _ are reserved for templates' };
...
export async function createAgent(opts) {
    ...
    if (existing.length >= 20)
        throw new Error('Maximum of 20 agents reached. Delete unused agents first.');
    // Validate token
    const tokenCheck = await validateBotToken(botToken);
    if (!tokenCheck.ok || !tokenCheck.botInfo)
    ...
    fs.writeFileSync(path.join(agentDir, 'agent.yaml'), yaml.dump(agentYaml, { lineWidth: -1 }), 'utf-8');
...
// ── .env management ──────────────────────────────────────────────────
function writeBotTokenToEnv(envPath, envKey, token, agentId) {
    let content = '';
    try {
...
function removeBotTokenFromEnv(envPath, envKey, agentId) {
...
function generateServiceConfig(agentId) {
...
function generateLaunchdPlist(agentId) {
...
function generateSystemdUnit(agentId) {
...
function activateLaunchd(agentId) {
    const label = `com.claudeclaw.${agentId}`;
    ...
    try {
        if (os.platform() === 'darwin') {
            ...
            if (fs.existsSync(destPlist)) {
                try {
                    execSync(`launchctl unload "${destPlist}"`, { stdio: 'ignore' });
                }
                catch { /* ok */ }
...
export function deleteAgent(agentId) {
    ...
    try {
        ...
        if (fs.existsSync(plistTemplate))
            fs.unlinkSync(plistTemplate);
...
export function suggestBotNames(agentId) {
...
export function pickAgentColor(existingCount) {
