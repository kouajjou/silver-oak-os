...
let client = null;
export async function initWhatsApp(onIncoming) {
    ...
    client.on('message', async (msg) => {
        if (msg.fromMe || msg.from === 'status@broadcast' || !msg.body)
            return;
        try {
...
export async function getWaChats(limit = 5) {
    if (!client)
...
export async function getWaChatMessages(chatId, limit = 10) {
    if (!client)
        throw new Error('WhatsApp not connected');
    ...
    return messages.map((msg) => ({
        ...
        senderName: msg.fromMe ? 'You' : (msg._data?.notifyName ?? chat.name),
