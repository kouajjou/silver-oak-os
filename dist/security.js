...
export function checkKillPhrase(message) {
    if (!_killPhrase)
        return false;
 ...
 */
export function executeEmergencyKill() {
            ...
            try {
                ...
                for (const line of output.split('\n')) {
                    ...
                    if (label && label.startsWith('com.claudeclaw.')) {
                        try {
                            execSync(`launchctl stop "${label}"`, { stdio: 'ignore', timeout: 2000 });
                        }
                        catch { /* ok */ }
                    }
                }
...
export function audit(entry) {
    if (_auditCallback) {
        ...
        catch { /* don't let audit failures block operations */ }
    }
