 ...
 */
export function loadMcpServers(allowlist, projectCwd) {
    const merged = {};
 ...
 */
export async function runAgent(message, sessionId, onTyping, onProgress, model, abortController, onStreamText, mcpAllowlist) {
    ...
    try {
        ...
        logger.info({ sessionId: sessionId ?? 'new', messageLen: message.length, mcpServers: mcpServerNames }, 'Starting agent query');
        ...
        for await (const event of query({
            ...
            if (ev['type'] === 'result') {
                ...
                logger.info({ hasResult: !!resultText, subtype: ev['subtype'] }, 'Agent result received');
            }
        }
    }
    catch (err) {
        ...
        logger.error({ category: classified.category, recovery: classified.recovery, originalMsg: err?.message }, 'Agent query failed (classified)');
        throw classified;
    }
    finally {
 ...
 */
export async function runAgentWithRetry(message, sessionId, onTyping, onProgress, model, abortController, onStreamText, onRetry, fallbackModels, mcpAllowlist) {
    let lastError;
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            const currentModel = attempt === 0 ? model
                : lastError?.recovery.shouldSwitchModel && fallbackModels?.length
                    ? fallbackModels[Math.min(attempt - 1, fallbackModels.length - 1)]
                    : model;
            return await runAgent(message, sessionId, onTyping, onProgress, currentModel, abortController, onStreamText, mcpAllowlist);
        }
        catch (err) {
            ...
            const delayMs = Math.min(BACKOFF_BASE_MS * Math.pow(BACKOFF_MULTIPLIER, attempt), 60000);
            // Add jitter (0-25% of delay)
            const jitter = Math.random() * delayMs * 0.25;
            logger.warn({ attempt: attempt + 1, category: err.category, delayMs: Math.round(delayMs + jitter) }, 'Retrying agent query');
