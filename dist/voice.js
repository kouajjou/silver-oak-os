...
// ── Upload directory ────────────────────────────────────────────────────────
export const UPLOADS_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'workspace', 'uploads');
 ...
 */
async function transcribeAudioGemini(filePath) {
 ...
 */
export async function downloadTelegramFile(botToken, fileId, destDir) {
...
async function transcribeAudioGroq(filePath) {
    ...
    const responseBuffer = await httpsRequest('https://api.groq.com/openai/v1/audio/transcriptions', {
        method: 'POST',
        headers: {
...
async function transcribeAudioLocal(filePath) {
    ...
    try {
        ...
        return (result.transcription || []).map((s) => s.text).join(' ').trim();
    }
    finally {
        try {
...
async function synthesizeSpeechElevenLabs(text) {
    ...
    return await httpsRequest(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
        method: 'POST',
        headers: {
            'xi-api-key': apiKey,
 ...
 */
async function synthesizeSpeechGradium(text) {
    const env = readEnvFile(['GRADIUM_API_KEY', 'GRADIUM_VOICE_ID']);
    const apiKey = env.GRADIUM_API_KEY;
    const voiceId = env.GRADIUM_VOICE_ID;
    if (!apiKey)
        throw new Error('GRADIUM_API_KEY not set');
    if (!voiceId)
        throw new Error('GRADIUM_VOICE_ID not set');
    const payload = JSON.stringify({
        text,
        voice_id: voiceId,
        output_format: 'opus',
    ...
    return await httpsRequest('https://eu.api.gradium.ai/api/post/speech/tts', {
        method: 'POST',
        headers: {
            'x-api-key': apiKey,
            'Content-Type': 'application/json',
 ...
 */
async function synthesizeSpeechKokoro(text) {
