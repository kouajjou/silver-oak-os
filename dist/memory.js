...
import { batchUpdateMemoryRelevance, decayMemories, getConsolidationsWithEmbeddings, getLastMemorySaveTime, getOtherAgentActivity, getRecentConsolidations, getRecentHighImportanceMemories, getTurnCountSinceTimestamp, logConversationTurn, pruneConversationLog, pruneSlackMessages, pruneWaMessages, searchConsolidations, searchConversationHistory, searchMemories, } from './db.js';
import { cosineSimilarity, embedText } from './embeddings.js';
import { generateContent, parseJsonResponse } from './gemini.js';
import { logger } from './logger.js';
import { ingestConversationTurn } from './memory-ingest.js';
import { buildObsidianContext } from './obsidian.js';
export async function buildMemoryContext(chatId, userMessage, agentId = 'main') {
    const seen = new Set();
 ...
 */
export function saveConversationTurn(chatId, userMessage, claudeResponse, sessionId, agentId = 'main') {
    try {
...
export function runDecaySweep() {
    ...
    if (wa.messages + wa.outbox + wa.map + slack > 0) {
        logger.info({ wa_messages: wa.messages, wa_outbox: wa.outbox, wa_map: wa.map, slack }, 'Retention pruning complete');
    }
}
/**
 ...
 */
export async function evaluateMemoryRelevance(surfacedMemoryIds, memorySummaries, userMessage, assistantResponse) {
    if (surfacedMemoryIds.length === 0 || !GOOGLE_API_KEY)
        return;
    try {
...
${memoryList}`;
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Evaluation timeout')), 5000));
        const raw = await Promise.race([generateContent(prompt), timeoutPromise]);
        const usefulIds = parseJsonResponse(raw);
        if (!usefulIds || !Array.isArray(usefulIds))
            return;
 ...
 */
export function shouldNudgeMemory(chatId, agentId = 'main') {
    ...
    // Time-based nudge
    if (hoursSinceSave >= MEMORY_NUDGE_INTERVAL_HOURS)
        return true;
    // Turn-based nudge
    const turnsSinceSave = getTurnCountSinceTimestamp(chatId, lastSave, agentId);
    if (turnsSinceSave >= MEMORY_NUDGE_INTERVAL_TURNS)
        return true;
    return false;
}
