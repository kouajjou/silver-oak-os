...
import { getUnconsolidatedMemories, saveConsolidationAtomic, } from './db.js';
...
function makeMemory(id, summary) {
    return {
        id,
        chat_id: 'chat1',
        source: 'conversation',
...
describe('runConsolidation', () => {
    ...
    it('consolidates 2+ memories and saves the result atomically', async () => {
        ...
        expect(mockSaveAtomic).toHaveBeenCalledWith('chat1', [10, 20, 30], consolidationResult.summary, consolidationResult.insight, 
    ...
    it('ignores connections with IDs outside the source set', async () => {
        ...
        expect(mockSaveAtomic).toHaveBeenCalledWith('chat1', [10, 20], 'summary', 'insight', [{ from_id: 10, to_id: 20, relationship: 'valid connection' }], []);
    });
    it('handles empty connections array', async () => {
        ...
        mockParseJson.mockReturnValue(result);
        await runConsolidation('chat1');
        expect(mockSaveAtomic).toHaveBeenCalledWith('chat1', [10, 20], result.summary, result.insight, [], []);
    });
    // ── Error handling ────────────────────────────────────────────────
...
//# sourceMappingURL=memory-consolidate.test.js.map