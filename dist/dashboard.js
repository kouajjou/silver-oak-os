...
import { getAllScheduledTasks, deleteScheduledTask, pauseScheduledTask, resumeScheduledTask, getConversationPage, getDashboardMemoryStats, getDashboardPinnedMemories, getDashboardLowSalienceMemories, getDashboardTopAccessedMemories, getDashboardMemoryTimeline, getDashboardConsolidations, getDashboardMemoriesList, getDashboardTokenStats, getDashboardCostTimeline, getDashboardRecentTokenUsage, getSession, getSessionTokenUsage, getHiveMindEntries, getAgentTokenStats, getAgentRecentConversation, getMissionTasks, getMissionTask, createMissionTask, cancelMissionTask, deleteMissionTask, reassignMissionTask, assignMissionTask, updateMissionTaskTimeout, getUnassignedMissionTasks, getMissionTaskHistory, getAuditLog, getAuditLogCount, getRecentBlockedActions, listActiveMeetSessions, listRecentMeetSessions, getMeetSession, createWarRoomMeeting, endWarRoomMeeting, addWarRoomTranscript, getWarRoomMeetings, getWarRoomTranscript, } from './db.js';
import { generateContent, parseJsonResponse } from './gemini.js';
import { getSecurityStatus } from './security.js';
import { listAgentIds, loadAgentConfig, setAgentModel, setAgentDescription, getMainDescription, setMainDescription, } from './agent-config.js';
import { listTemplates, validateAgentId, validateBotToken, createAgent, activateAgent, deactivateAgent, restartAgent, deleteAgent, suggestBotNames, isAgentRunning, } from './agent-create.js';
...
export function startDashboard(botApi) {
    if (!DASHBOARD_TOKEN) {
    ...
    app.get('/warroom-client.js', (c) => {
        ...
        if (!fs.existsSync(bundlePath))
            return c.text('// bundle not built', 404);
    ...
    app.post('/api/warroom/meeting/end', async (c) => {
        const body = await c.req.json().catch(() => ({}));
        if (body.id)
            endWarRoomMeeting(body.id, body.entryCount || 0);
        return c.json({ ok: true });
    });
    ...
    async function killWarroomAsync(reason) {
        try {
    ...
    app.post('/api/warroom/pin', async (c) => {
        ...
        try {
            fs.writeFileSync(WARROOM_PIN_PATH, JSON.stringify({ agent: nextAgent, mode: nextMode, pinnedAt: Date.now() }), 'utf-8');
    ...
    function writeVoicesFile(obj) {
        fs.writeFileSync(WARROOM_VOICES_PATH, JSON.stringify(obj, null, 2) + '\n', 'utf-8');
    }
    function pickDefaultGeminiVoice(used) {
    ...
    app.get('/api/warroom/voices', (c) => {
        ...
        const usedGeminiVoices = new Set(Object.values(configured)
            .map((v) => v && typeof v === 'object' ? v.gemini_voice : undefined)
    ...
    app.patch('/api/mission/tasks/:id', async (c) => {
        ...
        if (body?.timeout_ms !== undefined) {
            ...
            if (!changed)
                return c.json({ error: 'Task is no longer running' }, 422);
            if (!body?.assigned_agent)
                return c.json({ ok: true, timeout_ms: newTimeout });
        }
        const newAgent = body?.assigned_agent?.trim();
        if (!newAgent)
            return c.json({ error: 'assigned_agent required' }, 400);
        const validAgents = ['main', ...listAgentIds()];
        if (!validAgents.includes(newAgent))
    ...
    async function runMeetCli(args, timeoutMs) {
    ...
    app.get('/api/meet/sessions', (c) => {
        ...
        const recent = listRecentMeetSessions(15).filter((s) => s.status !== 'joining' && s.status !== 'live');
    ...
    app.post('/api/meet/join', async (c) => {
        ...
        if (!agent)
            return c.json({ ok: false, error: 'agent required' }, 400);
    ...
    app.post('/api/meet/join-daily', async (c) => {
        ...
        if (typeof ttlSec === 'number' && ttlSec > 0)
            args.push('--ttl-sec', String(ttlSec));
    ...
    app.get('/api/memories/list', (c) => {
        ...
        const offset = parseInt(c.req.query('offset') || '0', 10);
        const sortBy = (c.req.query('sort') || 'importance');
    ...
    app.get('/api/health', (c) => {
        ...
        if (sessionId) {
            ...
            if (summary) {
                ...
                if (ageSec < 3600)
                    sessionAge = Math.floor(ageSec / 60) + 'm';
                else if (ageSec < 86400)
                    sessionAge = Math.floor(ageSec / 3600) + 'h';
                else
                    sessionAge = Math.floor(ageSec / 86400) + 'd';
            }
        }
        return c.json({
            contextPct,
    ...
    app.get('/api/agents', (c) => {
        ...
        const agents = agentIds.map((id) => {
            try {
                ...
                try {
                    const envKey = config.botTokenEnv
    ...
    app.patch('/api/agents/:id/model', async (c) => {
        ...
        const validModels = ['claude-opus-4-6', 'claude-sonnet-4-6', 'claude-sonnet-4-5', 'claude-haiku-4-5'];
        if (!validModels.includes(model))
            return c.json({ error: `Invalid model. Valid: ${validModels.join(', ')}` }, 400);
        try {
            if (agentId === 'main') {
    ...
    app.post('/api/agents/:id/activate', (c) => {
        const agentId = c.req.param('id');
        if (agentId === 'main')
            return c.json({ error: 'Cannot activate main via this endpoint' }, 400);
    ...
    app.post('/api/agents/:id/deactivate', (c) => {
        const agentId = c.req.param('id');
        if (agentId === 'main')
            return c.json({ error: 'Cannot deactivate main via this endpoint' }, 400);
    ...
    app.post('/api/agents/:id/restart', (c) => {
        const agentId = c.req.param('id');
        if (agentId === 'main')
            return c.json({ error: 'Cannot restart main via this endpoint. Restart the main process manually.' }, 400);
    ...
    app.delete('/api/agents/:id/full', (c) => {
        const agentId = c.req.param('id');
        if (agentId === 'main')
            return c.json({ error: 'Cannot delete main' }, 400);
    ...
    app.post('/api/chat/send', async (c) => {
        if (!botApi)
            return c.json({ error: 'Bot API not available' }, 503);
        const body = await c.req.json();
                ...
                server.on('upgrade', (req, socket, head) => {
                    ...
                    wss.handleUpgrade(req, socket, head, (clientWs) => {
                        ...
                        remote.on('error', (err) => {
                            ...
                            try {
                                clientWs.close(1011, 'War Room server error');
                            }
                            catch { /* ok */ }
                        });
