...
beforeEach(() => {
    ...
    const memDb = mockedCtor.mock.results[0]?.value;
    if (memDb) {
        try {
            memDb.exec('DELETE FROM token_status');
        }
...
function fakeResponse(ok, status = ok ? 200 : 401) {
    return { ok, status };
}
describe('validateAllTokens', () => {
    ...
    it('marks Telegram bot token as invalid when API returns 401', async () => {
        ...
        mockedFetch.mockImplementation(async (url) => {
            if (url.includes('telegram.org/bot') && url.includes('getMe'))
                return fakeResponse(false, 401);
...
//# sourceMappingURL=token-validator.test.js.map