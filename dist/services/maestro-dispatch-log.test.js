...
import { logMaestroDispatch, cleanupOldDispatches, getDispatchStats, getRecentDispatches, } from './maestro-dispatch-log.js';
...
describe('logMaestroDispatch', () => {
    ...
    it('does NOT throw if insert internally fails (non-blocking guarantee)', () => {
        ...
        expect(() => 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        logMaestroDispatch({ user_id: null, mode: 'x', task: 't', success: true })).not.toThrow();
    });
});
describe('cleanupOldDispatches', () => {
    it('deletes rows older than 30 days', () => {
        ...
        db.prepare(`INSERT INTO maestro_dispatches (ts, user_id, mode, task_preview, success)
       VALUES (?, 'k', 'm', 'old', 1), (?, 'k', 'm', 'recent', 1)`).run(old, recent);
        const deleted = cleanupOldDispatches();
        expect(deleted).toBe(1);
        const remaining = db.prepare('SELECT task_preview FROM maestro_dispatches').all();
...
//# sourceMappingURL=maestro-dispatch-log.test.js.map