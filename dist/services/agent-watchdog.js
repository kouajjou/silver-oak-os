...
function readSystemdAgentState(agentId) {
    if (os.platform() !== "linux")
        return null;
    const serviceName = `com.claudeclaw.agent-${agentId}`;
    try {
        const output = execSync(`systemctl --user show "${serviceName}" --property=ActiveState,SubState,LoadState,NRestarts`, { encoding: "utf-8", env: getSystemdEnv(), stdio: "pipe" });
        const props = {};
        output.split("\n").forEach((line) => {
...
// Telegram alert ─────────────────────────────────────────────
async function sendCrashAlert(agentId, restartsCount, windowMin) {
    const token = TELEGRAM_BOT_TOKEN;
    const chatId = ALLOWED_CHAT_ID;
    if (!token || !chatId) {
        logger.warn({ agentId }, "[watchdog] CRASH LOOP detected but TELEGRAM_BOT_TOKEN or ALLOWED_CHAT_ID missing");
        return;
    }
    const text = `🚨 *CRASH LOOP* — agent \`${agentId}\`\n` +
...
function checkAgent(agentId) {
    ...
    if (!state)
        return; // not loaded, skip silently
    const now = Date.now();
    ...
    if (history.restarts_window.length >= 2) {
        ...
        if (restartDelta >= CRASH_THRESHOLD && !history.alerted) {
            logger.error({ agentId, restartDelta, window: WINDOW_MINUTES }, "[watchdog] CRASH LOOP DETECTED — stopping agent");
            // Stop + disable + alert
...
// List active agents ─────────────────────────────────────────
function listSystemdAgents() {
    if (os.platform() !== "linux")
        return [];
    try {
        const output = execSync(`systemctl --user list-units 'com.claudeclaw.agent-*' --no-pager --plain --no-legend`, { encoding: "utf-8", env: getSystemdEnv(), stdio: "pipe" });
        const agents = [];
        output.split("\n").forEach((line) => {
...
export function startAgentWatchdog() {
    ...
    logger.info({ interval_ms: CHECK_INTERVAL_MS, threshold: CRASH_THRESHOLD, window_min: WINDOW_MINUTES }, "[watchdog] Started");
...
export function stopAgentWatchdog() {
    if (watchdogInterval) {
