...
async function httpCheck(url, headers) {
...
async function checkTelegram(name, token) {
...
async function checkGemini(name, key) {
    const start = Date.now();
...
async function checkOpenAICompat(name, key, baseUrl) {
    const start = Date.now();
    if (!key || key.length < 20) {
        return { name, valid: false, error: 'key missing', response_ms: 0 };
    }
...
// ── Telegram alerte ─────────────────────────────────────────────────────
async function sendInvalidTokenAlert(invalidCritical) {
    if (!TELEGRAM_BOT_TOKEN || !ALLOWED_CHAT_ID || invalidCritical.length === 0)
        return;
    const lines = invalidCritical.map((t) => `• \`${t.name}\` — ${t.error ?? 'invalid'}`);
    const text = `⚠️ *Tokens invalides au boot*\n` +
        lines.join('\n') +
...
export async function validateAllTokens() {
    ...
    for (const k of [
        ...
        if (env[k])
            checks.push(checkTelegram(k, env[k]));
    }
    ...
    logger.info({ total: results.length, ok: okCount, fail: failCount }, '[token-validator] Boot validation complete');
    for (const r of results) {
        if (!r.valid) {
            logger.warn({ token: r.name, error: r.error, response_ms: r.response_ms }, '[token-validator] Invalid');
        }
    }
    ...
    if (invalidCritical.length > 0) {
        sendInvalidTokenAlert(invalidCritical).catch(() => { });
    }
    return results;
}
/**
 ...
 */
export function getTokenStatuses() {
    return db
