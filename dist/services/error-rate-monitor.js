...
`);
// ── Prepared statements ────────────────────────────────────────────────────
const stmtInsert = db.prepare("INSERT INTO request_log (agent_id, status_code, is_error, route) VALUES (?, ?, ?, ?)");
...
`);
const stmtCleanup = db.prepare("DELETE FROM request_log WHERE timestamp < unixepoch() - 86400");
// ── Public API ─────────────────────────────────────────────────────────────
/**
 ...
 */
export function getErrorRate(agent_id, windowMinutes = 5, thresholdPct = 30) {
    const row = stmtStats.get(agent_id, windowMinutes);
 ...
 */
export async function checkAndAlert(agent_id, windowMinutes = 5, thresholdPct = 30) {
    const result = getErrorRate(agent_id, windowMinutes, thresholdPct);
    if (result.exceeded) {
        const token = process.env.TELEGRAM_BOT_TOKEN;
        const chatId = process.env.ALLOWED_CHAT_ID;
        if (token && chatId) {
            const text = `🚨 *Error Rate Alert* — \`${agent_id}\`\n` +
 ...
 */
export function getAllAgentsErrorRates(windowMinutes = 5, thresholdPct = 30) {
    const agents = stmtAllAgents.all(windowMinutes);
...
//# sourceMappingURL=error-rate-monitor.js.map