...
`);
const stmtInsertCost = db.prepare('INSERT INTO agent_costs (agent_id, cost_usd, tokens_in, tokens_out, model) VALUES (?, ?, ?, ?, ?)');
const stmtDailyTotal = db.prepare('SELECT COALESCE(SUM(cost_usd), 0) AS total FROM agent_costs WHERE agent_id = ? AND timestamp > ?');
 ...
 */
export function getAgentDailyTotal(agent_id) {
    const oneDayAgo = Math.floor(Date.now() / 1000) - 86400;
    const row = stmtDailyTotal.get(agent_id, oneDayAgo);
    return row.total;
}
/**
 * Get total cost for an agent in the last 30 days.
 */
export function getAgentMonthlyTotal(agent_id) {
    const oneMonthAgo = Math.floor(Date.now() / 1000) - 2592000;
    const row = stmtMonthlyTotal.get(agent_id, oneMonthAgo);
    return row.total;
}
/**
 ...
 */
export function setAgentBudget(agent_id, daily_cap_usd, monthly_cap_usd) {
 ...
 */
export function checkBudgetStatus(agent_id) {
    const budget = stmtGetBudget.get(agent_id);
 ...
 */
export function getAllAgentsCosts() {
    const oneDayAgo = Math.floor(Date.now() / 1000) - 86400;
    const oneMonthAgo = Math.floor(Date.now() / 1000) - 2592000;
    return db.prepare(`
    SELECT
