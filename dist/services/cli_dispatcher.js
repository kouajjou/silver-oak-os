 ...
 */
export function selectSession(taskDescription) {
    const lower = taskDescription.toLowerCase();
    const isComplex = /architect|refactor|redesign|strateg|autonomous|orchestrat|vision|migration|big.bang|strangler/i.test(lower);
    if (isComplex)
        return 'opus';
    const isFrontend = /frontend|react|next\.?js|\bui\b|component|tailwind|css|\bpage\b|layout|design.system/i.test(lower);
    if (isFrontend)
        return 'claude-frontend';
    const isBackend = /backend|api|route|endpoint|express|hono|middleware|postgres|supabase|redis|bullmq|worker|stripe|payment|billing|webhook/i.test(lower);
    if (isBackend)
...
async function mcpInit() {
    ...
    if (!sessionId)
        throw new Error('MCP init failed: no mcp-session-id');
    return sessionId;
}
...
async function mcpCallTool(mcpSessionId, toolName, args) {
    const resp = await axios.post(`${MCP_BRIDGE_URL}/mcp`, { jsonrpc: '2.0', method: 'tools/call', id: Date.now(), params: { name: toolName, arguments: args } }, {
        headers: {
 ...
 */
export async function dispatchToCLI(opts) {
...
TASK_DONE_CLI_DISPATCH`;
    logger.info({ session: opts.session, promptLen: fullPrompt.length, timeout, pollMs }, 'cli_dispatcher.send');
    // 1. Send prompt to session via /dispatch
    await axios.post(`${MCP_BRIDGE_URL}/dispatch`, { session: opts.session, prompt: fullPrompt, priority: 'high' }, { timeout: 30_000 });
    ...
    while (Date.now() - start < timeout) {
        ...
        const hasMarker = output.includes('TASK_DONE_CLI_DISPATCH') ||
            /TASK_DONE_[A-Z_]+/i.test(output);
        logger.debug({ session: opts.session, elapsed: Date.now() - start, hasMarker }, 'cli_dispatcher.poll');
        if (hasMarker) {
            taskDone = true;
    ...
    logger.info({ session: opts.session, taskDone, duration: Date.now() - start }, taskDone ? 'cli_dispatcher.done' : 'cli_dispatcher.timeout');
    return {
        success: taskDone,
        output,
        session: opts.session,
...
//# sourceMappingURL=cli_dispatcher.js.map