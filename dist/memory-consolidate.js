...
import { getUnconsolidatedMemories, saveConsolidationAtomic, saveConsolidationEmbedding, } from './db.js';
...
const consolidatingChats = new Set();
/**
 ...
 */
export async function runConsolidation(chatId) {
    ...
    try {
        ...
        const prompt = CONSOLIDATION_PROMPT.replace('{MEMORIES}', JSON.stringify(memoriesJson, null, 2));
        ...
        if (result.connections && result.connections.length > 0) {
            for (const conn of result.connections) {
                if (!conn.from_id || !conn.to_id)
                    continue;
                if (!sourceIds.includes(conn.from_id) || !sourceIds.includes(conn.to_id))
                    continue;
        ...
        if (result.contradictions && result.contradictions.length > 0) {
            for (const contra of result.contradictions) {
                if (!sourceIds.includes(contra.stale_id) || !sourceIds.includes(contra.supersedes_id)) {
                    logger.warn({ staleId: contra.stale_id, supersededBy: contra.supersedes_id, reason: contra.reason }, 'Contradiction detected but IDs not in current batch, skipping');
                    continue;
                }
                ...
                if (staleMem && newMem && staleMem.created_at > newMem.created_at) {
                    ...
                    logger.warn({ originalStale: contra.stale_id, correctedStale: staleId }, 'Corrected contradiction direction (LLM assigned newer memory as stale)');
                }
                validContradictions.push({ stale_id: staleId, superseded_by: supersededBy });
                logger.info({ staleId, supersededBy, reason: contra.reason }, 'Memory superseded (contradiction resolved)');
            }
        }
        ...
        const consolidationId = saveConsolidationAtomic(chatId, sourceIds, result.summary, result.insight, validConnections, validContradictions);
