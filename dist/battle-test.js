 ...
 */
import { initDatabase, getSession, setSession, clearSession, createScheduledTask, getAllScheduledTasks, saveTokenUsage, getAgentTokenStats, logToHiveMind, getHiveMindEntries, logConversationTurn, } from './db.js';
initDatabase();
let passed = 0;
let failed = 0;
...
assert('Ops tasks only contain ops agent_id', opsTasks.every((t) => t.agent_id === 'ops'));
assert('Research tasks only contain research agent_id', resTasks.every((t) => t.agent_id === 'research'));
...
assert('Ops hive all have ops agent_id', opsHive.every((e) => e.agent_id === 'ops'));
