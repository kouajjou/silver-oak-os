...
describe('checkPendingMigrations', () => {
    ...
    function writeVersionJson(versions) {
        ...
        fs.writeFileSync(path.join(migrationsDir, 'version.json'), JSON.stringify({ migrations: versions }, null, 2));
    }
    function writeAppliedJson(lastApplied) {
        const migrationsDir = path.join(tmpDir, 'migrations');
        fs.mkdirSync(migrationsDir, { recursive: true });
        fs.writeFileSync(path.join(migrationsDir, '.applied.json'), JSON.stringify({ lastApplied }, null, 2));
    }
    ...
    describe('fresh clone', () => {
        ...
        it('picks the highest version when multiple versions exist', () => {
            ...
            checkPendingMigrations(tmpDir);
            const state = JSON.parse(fs.readFileSync(path.join(tmpDir, 'migrations', '.applied.json'), 'utf-8'));
    ...
    describe('pending migrations', () => {
        ...
        it('includes applied and latest versions in the error message', () => {
            ...
            writeAppliedJson('v1.0.0');
            checkPendingMigrations(tmpDir);
            const msg = vi.mocked(console.error).mock.calls[0]?.[0];
