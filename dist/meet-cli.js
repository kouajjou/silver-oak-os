...
import { initDatabase, createMeetSession, markMeetSessionLive, markMeetSessionLeft, markMeetSessionFailed, getMeetSession, listActiveMeetSessions, listRecentMeetSessions, } from './db.js';
...
const PIKA_SCRIPT = path.join(PROJECT_ROOT, 'skills', 'pikastream-video-meeting', 'scripts', 'pikastreaming_videomeeting.py');
...
function resolveAvatarPath(agentId) {
...
function resolveAgent(agentId) {
    ...
    if (agentId !== 'main') {
        try {
            ...
            if (cfg.meetBotName)
                botName = cfg.meetBotName;
            else if (cfg.name)
                botName = cfg.name;
            if (cfg.meetVoiceId)
                voiceId = cfg.meetVoiceId;
        }
        catch (err) {
...
async function runPikaScript(args, timeoutSec = 90) {
    if (!fs.existsSync(PIKA_SCRIPT)) {
        throw new Error(`Pika script not found at ${PIKA_SCRIPT}`);
    }
    if (!fs.existsSync(WARROOM_VENV_PY)) {
        throw new Error(`Warroom venv python not found at ${WARROOM_VENV_PY}. Create it with: ` +
    ...
    const env = { ...process.env, PIKA_DEV_KEY: devKey };
    for (const k of [
        'CLAUDECODE',
...
function buildMinimalBrief(params) {
...
async function cmdJoin() {
    ...
    if (!resolved.imagePath) {
        die(`No avatar found for agent '${agentId}'. Expected one of: ` +
    ...
    if (autoBrief && !briefPath) {
        ...
        process.stderr.write(`[meet-cli] auto-brief: ${briefResult.fallback ? 'FALLBACK minimal brief' : 'full brief'} ` +
    ...
    if (briefPath)
        pikaArgs.push('--system-prompt-file', briefPath);
    if (meetingPassword)
        pikaArgs.push('--meeting-password', meetingPassword);
...
async function cmdJoinDaily() {
    ...
    if (!fs.existsSync(WARROOM_VENV_PY)) {
        die(`warroom venv python not found at ${WARROOM_VENV_PY}. Create it with: ` +
            `python3 -m venv warroom/.venv && warroom/.venv/bin/pip install -r warroom/requirements.txt`);
    }
    ...
    if (briefPath)
        agentArgs.push('--brief', briefPath);
...
async function cmdLeave() {
    ...
    if (!existing)
        die(`no session found: ${sessionId}`);
...
//# sourceMappingURL=meet-cli.js.map