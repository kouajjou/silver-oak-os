...
export function checkPendingMigrations(projectRoot) {
    ...
    try {
        const registry = JSON.parse(fs.readFileSync(versionFile, 'utf-8'));
        ...
        const hasPending = lastApplied === null || compareSemver(lastApplied, latest) < 0;
        if (hasPending) {
