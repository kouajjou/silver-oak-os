...
export function getWorker(session) {
    return WORKERS.find(w => w.session === session);
}
export function getActiveWorkersByTier(tier) {
    return WORKERS.filter(w => w.tier === tier && !w.banned);
}
export function getCheapestWorkerForUseCase(useCase) {
...
export function getBannedWorkers() {
    return WORKERS.filter(w => w.banned);
}
//# sourceMappingURL=workers-config.js.map