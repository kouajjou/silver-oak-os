...
export let activeBotToken = process.env.TELEGRAM_BOT_TOKEN || envConfig.TELEGRAM_BOT_TOKEN || '';
export let agentCwd; // undefined = use PROJECT_ROOT
export let agentDefaultModel; // from agent.yaml
export let agentObsidianConfig;
export let agentSystemPrompt; // loaded from agents/{id}/CLAUDE.md
export let agentMcpAllowlist; // from agent.yaml mcp_servers
export let agentSkillsAllowlist; // from agent.yaml skills_allowlist
...
export const ALLOWED_CHAT_ID = process.env.ALLOWED_CHAT_ID || envConfig.ALLOWED_CHAT_ID || '';
export const MESSENGER_TYPE = (process.env.MESSENGER_TYPE || envConfig.MESSENGER_TYPE || 'telegram').toLowerCase();
...
export const SIGNAL_RPC_HOST = process.env.SIGNAL_RPC_HOST || envConfig.SIGNAL_RPC_HOST || '127.0.0.1';
export const SIGNAL_RPC_PORT = parseInt(process.env.SIGNAL_RPC_PORT || envConfig.SIGNAL_RPC_PORT || '7583', 10);
...
export const SIGNAL_AUTHORIZED_RECIPIENTS = (process.env.SIGNAL_AUTHORIZED_RECIPIENTS || envConfig.SIGNAL_AUTHORIZED_RECIPIENTS || '').split(',').map((s) => s.trim()).filter(Boolean);
export const WHATSAPP_ENABLED = (process.env.WHATSAPP_ENABLED || envConfig.WHATSAPP_ENABLED || '').toLowerCase() === 'true';
export const SLACK_USER_TOKEN = process.env.SLACK_USER_TOKEN || envConfig.SLACK_USER_TOKEN || '';
...
const rawConfigDir = process.env.CLAUDECLAW_CONFIG || envConfig.CLAUDECLAW_CONFIG || '~/.claudeclaw';
/**
...
export const AGENT_TIMEOUT_MS = parseInt(process.env.AGENT_TIMEOUT_MS || envConfig.AGENT_TIMEOUT_MS || '1800000', 10);
...
export const MISSION_TIMEOUT_MS = Math.max(60_000, parseInt(process.env.MISSION_TIMEOUT_MS || envConfig.MISSION_TIMEOUT_MS || '900000', 10));
...
export const AGENT_MAX_TURNS = parseInt(process.env.AGENT_MAX_TURNS || envConfig.AGENT_MAX_TURNS || '30', 10);
...
export const CONTEXT_LIMIT = parseInt(process.env.CONTEXT_LIMIT || envConfig.CONTEXT_LIMIT || '1000000', 10);
// Dashboard — web UI for monitoring ClaudeClaw state
export const DASHBOARD_PORT = parseInt(process.env.DASHBOARD_PORT || envConfig.DASHBOARD_PORT || '3141', 10);
export const DASHBOARD_TOKEN = process.env.DASHBOARD_TOKEN || envConfig.DASHBOARD_TOKEN || '';
export const DASHBOARD_URL = process.env.DASHBOARD_URL || envConfig.DASHBOARD_URL || '';
// Database encryption key (SQLCipher). Required for encrypted database access.
export const DB_ENCRYPTION_KEY = process.env.DB_ENCRYPTION_KEY || envConfig.DB_ENCRYPTION_KEY || '';
...
export const STREAM_STRATEGY = (process.env.STREAM_STRATEGY || envConfig.STREAM_STRATEGY || 'off');
// ── Security ─────────────────────────────────────────────────────────
...
export const SECURITY_PIN_HASH = process.env.SECURITY_PIN_HASH || envConfig.SECURITY_PIN_HASH || '';
// Auto-lock after N minutes of inactivity. 0 = disabled. Only active when PIN is set.
export const IDLE_LOCK_MINUTES = parseInt(process.env.IDLE_LOCK_MINUTES || envConfig.IDLE_LOCK_MINUTES || '0', 10);
// Emergency kill phrase. Sending this to any bot immediately stops all agents and exits.
export const EMERGENCY_KILL_PHRASE = process.env.EMERGENCY_KILL_PHRASE || envConfig.EMERGENCY_KILL_PHRASE || '';
...
export const MODEL_FALLBACK_CHAIN = (process.env.MODEL_FALLBACK_CHAIN || envConfig.MODEL_FALLBACK_CHAIN || '').split(',').map((s) => s.trim()).filter(Boolean);
// Smart model routing: route simple messages to a cheap model.
// Defaults to false to preserve existing behavior. Opt in via .env.
export const SMART_ROUTING_ENABLED = (process.env.SMART_ROUTING_ENABLED || envConfig.SMART_ROUTING_ENABLED || 'false').toLowerCase() === 'true';
export const SMART_ROUTING_CHEAP_MODEL = process.env.SMART_ROUTING_CHEAP_MODEL || envConfig.SMART_ROUTING_CHEAP_MODEL || 'claude-haiku-4-5';
export const SHOW_COST_FOOTER = (process.env.SHOW_COST_FOOTER || envConfig.SHOW_COST_FOOTER || 'compact');
// Memory notifications: send Telegram message when high-importance memories are created.
// Set to 'off' to disable. Default: 'on'.
export const MEMORY_NOTIFY = (process.env.MEMORY_NOTIFY || envConfig.MEMORY_NOTIFY || 'on') !== 'off';
...
export const DAILY_COST_BUDGET = parseFloat(process.env.DAILY_COST_BUDGET || envConfig.DAILY_COST_BUDGET || '0');
// Hourly token budget. Warns at 80%. Set to 0 to disable (default).
export const HOURLY_TOKEN_BUDGET = parseInt(process.env.HOURLY_TOKEN_BUDGET || envConfig.HOURLY_TOKEN_BUDGET || '0', 10);
// Memory nudge intervals
export const MEMORY_NUDGE_INTERVAL_TURNS = parseInt(process.env.MEMORY_NUDGE_INTERVAL_TURNS || envConfig.MEMORY_NUDGE_INTERVAL_TURNS || '10', 10);
export const MEMORY_NUDGE_INTERVAL_HOURS = parseInt(process.env.MEMORY_NUDGE_INTERVAL_HOURS || envConfig.MEMORY_NUDGE_INTERVAL_HOURS || '2', 10);
// Secret exfiltration guard
export const EXFILTRATION_GUARD_ENABLED = (process.env.EXFILTRATION_GUARD_ENABLED || envConfig.EXFILTRATION_GUARD_ENABLED || 'true').toLowerCase() === 'true';
export const PROTECTED_ENV_VARS = (process.env.PROTECTED_ENV_VARS || envConfig.PROTECTED_ENV_VARS ||
    'ANTHROPIC_API_KEY,CLAUDE_CODE_OAUTH_TOKEN,DB_ENCRYPTION_KEY,TELEGRAM_BOT_TOKEN,SLACK_USER_TOKEN,GROQ_API_KEY,ELEVENLABS_API_KEY,GOOGLE_API_KEY').split(',').map((s) => s.trim()).filter(Boolean);
// ── War Room (voice meeting via Pipecat WebSocket) ──────────────────
export const WARROOM_ENABLED = (process.env.WARROOM_ENABLED || envConfig.WARROOM_ENABLED || 'false').toLowerCase() === 'true';
export const WARROOM_PORT = parseInt(process.env.WARROOM_PORT || envConfig.WARROOM_PORT || '7860', 10);
