 ...
 */
export function scanForSecrets(text, protectedValues) {
    const matches = [];
    const seen = new Set(); // dedupe by "position:length"
 ...
 */
export function redactSecrets(text, matches) {
    if (matches.length === 0)
        return text;
