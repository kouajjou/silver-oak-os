...
import { AGENT_ID, AGENT_TIMEOUT_MS, DASHBOARD_PORT, DASHBOARD_TOKEN, DASHBOARD_URL, EXFILTRATION_GUARD_ENABLED, MODEL_FALLBACK_CHAIN, PROJECT_ROOT, PROTECTED_ENV_VARS, SHOW_COST_FOOTER, SIGNAL_AUTHORIZED_RECIPIENTS, SIGNAL_PHONE_NUMBER, SIGNAL_RPC_HOST, SIGNAL_RPC_PORT, SMART_ROUTING_CHEAP_MODEL, SMART_ROUTING_ENABLED, agentDefaultModel, agentMcpAllowlist, agentSystemPrompt, } from './config.js';
import { buildCostFooter } from './cost-footer.js';
import { clearSession, getRecentMemories, getRecentTaskOutputs, getSession, pinMemory, setSession, unpinMemory, } from './db.js';
import { scanForSecrets, redactSecrets } from './exfiltration-guard.js';
import { logger } from './logger.js';
import { MEMORY_NUDGE_TEXT, buildMemoryContext, evaluateMemoryRelevance, saveConversationTurn, shouldNudgeMemory, } from './memory.js';
...
import { audit, checkKillPhrase, executeEmergencyKill, getSecurityStatus, isLocked, lock, touchActivity, unlock, } from './security.js';
...
function tryUnlink(p) {
    try {
        ...
        logger.warn({ err, path: p }, 'tmp unlink failed');
    }
}
 ...
 */
function resolveSignalAttachmentPath(att) {
...
function isAuthorised(sourceNumber) {
...
function splitMessage(text) {
    if (text.length <= SIGNAL_MAX_MESSAGE_LENGTH)
        return [text];
    const parts = [];
    let remaining = text;
...
export function createSignalBot() {
    if (!SIGNAL_PHONE_NUMBER) {
        throw new Error('SIGNAL_PHONE_NUMBER not set in .env. Link signal-cli first, then configure it.');
    }
    ...
    const sendFile = async (recipient, filePath, caption) => {
    ...
    const sendTyping = async (recipient) => {
        try {
     ...
     */
    async function handleTextMessage(incoming, message, forceVoiceReply = false) {
        ...
        if (delegation) {
            ...
            try {
                const result = await delegateToAgent(delegation.agentId, delegation.prompt, chatId, AGENT_ID, async (progressMsg) => {
        ...
        if (shouldNudgeMemory(chatId, AGENT_ID))
        ...
        try {
            const onProgress = (event) => {
                ...
                if (event.type === 'task_started')
                    void sendMessage(chatId, `🔄 ${event.description}`);
            ...
            const result = await runAgentWithRetry(fullMessage, sessionId, () => void sendTyping(chatId), onProgress, effectiveModel, abortCtrl, 
            /* onStreamText */ undefined, async (attempt, error) => {
            ...
            const shouldSpeakBack = caps.tts && (mode === 'on' || (mode !== 'off' && forceVoiceReply));
            if (textWithFooter) {
    ...
    async function handleCommand(incoming, text) {
        const chatId = incoming.sourceNumber;
        ...
        switch (cmd) {
            case 'start':
                ...
                return true;
            case 'help':
                await sendMessage(chatId, 'ClaudeClaw — Commands (Signal)\n\n' +
            ...
            case 'pin': {
                ...
                if (!id) {
                    await sendMessage(chatId, 'Usage: /pin <memory_id>');
                    return true;
                }
                pinMemory(id);
            ...
            case 'unpin': {
                const id = parseInt(arg, 10);
                if (!id) {
                    await sendMessage(chatId, 'Usage: /unpin <memory_id>');
                    return true;
                }
                unpinMemory(id);
            ...
            case 'voice': {
                const caps = voiceCapabilities();
                if (!caps.tts) {
                    await sendMessage(chatId, 'Voice replies not available. Configure one of:\n' +
            ...
            case 'model': {
                ...
                if (!target) {
                    await sendMessage(chatId, 'Unknown model. Use opus, sonnet, or haiku.');
                    return true;
                }
            ...
            case 'dashboard': {
                ...
                if (!token) {
                    await sendMessage(chatId, 'Dashboard not configured (DASHBOARD_TOKEN missing).');
                    return true;
                }
            ...
            case 'delegate': {
                ...
                const rest = arg;
                if (!rest) {
                    await sendMessage(chatId, 'Usage: /delegate <agent> <prompt>');
                    return true;
                }
    ...
    const onMessage = async (incoming) => {
        ...
        if (incoming.isSync) {
            if (incoming.destinationNumber !== SIGNAL_PHONE_NUMBER)
                return;
        ...
        const voiceAttachment = incoming.attachments.find((a) => a.contentType?.startsWith('audio/') ?? false);
        if (voiceAttachment) {
            ...
            if (!resolvedPath || !fs.existsSync(resolvedPath)) {
                logger.warn({ attachmentId: voiceAttachment.id, contentType: voiceAttachment.contentType, tried: resolvedPath }, 'Voice attachment path could not be resolved');
                await sendMessage(incoming.sourceNumber, 'Got your voice note but could not locate the audio file on disk. Check signal-cli attachment-storage config.');
                return;
            }
            voiceAttachment.path = resolvedPath;
            const caps = voiceCapabilities();
            if (!caps.stt) {
                await sendMessage(incoming.sourceNumber, 'Voice transcription not configured. Set GROQ_API_KEY for cloud STT or install whisper-cpp for local STT.');
                return;
            }
        ...
        if (!text && incoming.attachments.length === 0)
            return;
    ...
    return {
        async start() {
            ...
            logger.info({ host: SIGNAL_RPC_HOST, port: SIGNAL_RPC_PORT, account: SIGNAL_PHONE_NUMBER }, 'Signal bot connected to signal-cli daemon');
        },
        async stop() {
            rpc.stop();
        },
        async sendTo(recipient, text) {
...
export { splitMessage };
//# sourceMappingURL=signal-bot.js.map