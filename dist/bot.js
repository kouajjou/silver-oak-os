...
import { AGENT_ID, ALLOWED_CHAT_ID, CONTEXT_LIMIT, DASHBOARD_PORT, DASHBOARD_TOKEN, DASHBOARD_URL, MAX_MESSAGE_LENGTH, activeBotToken, agentDefaultModel, agentMcpAllowlist, agentSkillsAllowlist, agentSystemPrompt, TYPING_REFRESH_MS, AGENT_TIMEOUT_MS, STREAM_STRATEGY, MODEL_FALLBACK_CHAIN, SHOW_COST_FOOTER, SMART_ROUTING_ENABLED, SMART_ROUTING_CHEAP_MODEL, EXFILTRATION_GUARD_ENABLED, PROTECTED_ENV_VARS, DAILY_COST_BUDGET, HOURLY_TOKEN_BUDGET, MEMORY_NOTIFY, PROJECT_ROOT, } from './config.js';
import { clearSession, createMissionTask, getMissionTask, getRecentConversation, getRecentMemories, getRecentTaskOutputs, getSession, getSessionConversation, logToHiveMind, pinMemory, unpinMemory, setSession, lookupWaChatId, saveTokenUsage, saveCompactionEvent, getCompactionCount } from './db.js';
...
import { isLocked, lock, unlock, touchActivity, checkKillPhrase, executeEmergencyKill, isSecurityEnabled, getSecurityStatus, audit, } from './security.js';
// ── Streaming rate limiter ───────────────────────────────────────────
const globalStreamLastEdit = new Map();
...
const sessionBaseline = new Map(); // sessionId -> first turn's input_tokens
/**
 ...
 */
function checkContextWarning(chatId, sessionId, usage) {
...
import { downloadTelegramFile, transcribeAudio, synthesizeSpeech, voiceCapabilities, UPLOADS_DIR, } from './voice.js';
import { getSlackConversations, getSlackMessages, sendSlackMessage } from './slack.js';
import { getWaChats, getWaChatMessages, sendWhatsAppMessage } from './whatsapp.js';
...
export function setMainModelOverride(model) {
    if (ALLOWED_CHAT_ID)
        chatModelOverride.set(ALLOWED_CHAT_ID, model);
}
const waState = new Map();
const slackState = new Map();
/**
 ...
 */
function extractSelectionNumber(text) {
...
export function extractFileMarkers(text) {
    ...
    const cleaned = text.replace(pattern, (_, kind, filePath, caption) => {
        files.push({
 ...
 */
async function sendTyping(api, chatId) {
    try {
...
async function replyIfLocked(ctx) {
 ...
 */
async function handleMessage(ctx, message, forceVoiceReply = false, skipLog = false) {
    ...
    if (!ALLOWED_CHAT_ID) {
        ...
        try {
            ...
            await ctx.reply(`Setup complete! Your chat ID (${chatId}) has been saved.\n\nRestarting now...`);
    ...
    logger.info({ chatId, messageLen: message.length }, 'Processing message');
    ...
    if (delegation) {
        ...
        try {
            const delegationResult = await delegateToAgent(delegation.agentId, delegation.prompt, chatIdStr, AGENT_ID, (progressMsg) => {
    ...
    try {
        ...
        const onStreamText = streamingEnabled ? (accumulated) => {
            const now = Date.now();
            const globalLast = globalStreamLastEdit.get(chatIdStr) ?? 0;
            const deltaLen = accumulated.length - lastEditLength;
            if (now - globalLast < GLOBAL_STREAM_INTERVAL_MS || deltaLen < 20)
                return;
        ...
        const result = await runAgentWithRetry(fullMessage, sessionId, () => void sendTyping(ctx.api, chatId), onProgress, effectiveModel, abortCtrl, onStreamText, (attempt, error) => {
            void ctx.reply(`${error.recovery.userMessage} (retry ${attempt}/${2})`).catch(() => { });
        }, MODEL_FALLBACK_CHAIN.length > 0 ? MODEL_FALLBACK_CHAIN : undefined, agentMcpAllowlist);
        ...
        if (EXFILTRATION_GUARD_ENABLED) {
            ...
            if (secretMatches.length > 0) {
                ...
                logger.warn({ matchCount: secretMatches.length, types: secretMatches.map((m) => m.type) }, 'Exfiltration guard: redacted secrets from response');
            }
        }
        ...
        if (result.usage) {
            ...
            try {
                saveTokenUsage(chatIdStr, activeSessionId, result.usage.inputTokens, result.usage.outputTokens, result.usage.lastCallCacheRead, result.usage.lastCallCacheRead + result.usage.lastCallInputTokens, result.usage.totalCostUsd, result.usage.didCompact, AGENT_ID);
            }
            catch (dbErr) {
            ...
            if (result.usage.didCompact && activeSessionId) {
                saveCompactionEvent(activeSessionId, result.usage.preCompactTokens ?? 0, result.usage.lastCallInputTokens, 0);
        ...
        if (err instanceof AgentError) {
            logger.error({ category: err.category, recovery: err.recovery }, 'Agent error (classified)');
 ...
 */
function discoverSkillCommands(allowlist) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (!/user_invocable:\s*true/i.test(fm))
                continue;
            // Extract name
 ...
 */
export async function sendHITLRequest(api, ticketId, question, options) {
    if (!ALLOWED_CHAT_ID) {
...
export function createBot() {
    ...
    const userInvocableSkills = new Set(skillCommands.map((s) => s.command));
    ...
    bot.command('help', (ctx) => {
        if (!isAuthorised(ctx.chat.id))
            return;
        return ctx.reply('ClaudeClaw — Commands\n\n' +
    ...
    bot.command('chatid', (ctx) => {
        if (ALLOWED_CHAT_ID)
            return; // Already configured — don't respond to anyone
        return ctx.reply(`Your chat ID: ${ctx.chat.id}`);
    });
    // /start — simple greeting (auth-gated after setup)
    bot.command('start', (ctx) => {
        if (ALLOWED_CHAT_ID && !isAuthorised(ctx.chat.id))
            return;
        if (AGENT_ID !== 'main') {
        ...
        if (oldSessionId) {
            ...
            (async () => {
                try {
                    ...
                    const result = await runAgent('Summarize what we accomplished this session in ONE short sentence (under 100 chars). No preamble, no quotes, just the summary. Example: "Drafted LinkedIn post about AI agents and scheduled Gmail triage task"', sessionToSummarize, () => { }, // no typing indicator
                    undefined, undefined, summaryAbort);
    ...
    bot.command('wa', async (ctx) => {
        ...
        try {
            ...
            await ctx.reply(`📱 <b>WhatsApp</b>\n\n${lines}\n\n<i>Send a number to open • r &lt;num&gt; &lt;text&gt; to reply</i>`, { parse_mode: 'HTML' });
        }
        catch (err) {
    ...
    bot.command('agents', async (ctx) => {
        ...
        await ctx.reply(`<b>Available agents</b>\n\n${lines}\n\n<i>Usage: @agentId: prompt or /delegate agentId prompt</i>`, { parse_mode: 'HTML' });
    });
    ...
    const OWN_COMMANDS = new Set(['/start', '/help', '/newchat', '/respin', '/voice', '/model', '/memory', '/forget', '/pin', '/unpin', '/chatid', '/wa', '/slack', '/dashboard', '/stop', '/agents', '/delegate', '/lock', '/status', '/test_hitl']);
    bot.on('message:text', async (ctx) => {
        const text = ctx.message.text;
        const chatIdStr = ctx.chat.id.toString();
        if (text.startsWith('/')) {
            const firstToken = text.split(/[\s@]/)[0].toLowerCase();
            if (OWN_COMMANDS.has(firstToken))
                return; // already handled by bot.command() above
        ...
        if (state?.mode === 'list' && waSelection !== null) {
            ...
            if (idx >= 0 && idx < state.chats.length) {
                ...
                try {
                    ...
                    await ctx.reply(`💬 <b>${escapeHtml(target.name)}</b>\n\n${lines}\n\n<i>r &lt;text&gt; to reply • /wa to go back</i>`, { parse_mode: 'HTML' });
                }
                catch (err) {
        ...
        if (slkState?.mode === 'list' && slackSelection !== null) {
            ...
            if (idx >= 0 && idx < slkState.convos.length) {
                ...
                try {
                    ...
                    await ctx.reply(`${icon} <b>${escapeHtml(target.name)}</b>\n\n${lines}\n\n<i>r &lt;text&gt; to reply • /slack to go back</i>`, { parse_mode: 'HTML' });
                }
                catch (err) {
    ...
    bot.command("test_hitl", async (ctx) => {
        ...
        try {
            await sendHITLRequest(bot.api, ticketId, "🧪 Test HITL gap-005 — Boutons inline Telegram (MVPstratégie A). Clique un bouton :", ["Approve", "Reject"]);
 ...
 */
export function dispatchDashboardChatToAgent(text, targetAgentId) {
    if (!ALLOWED_CHAT_ID)
        return;
    ...
    logger.info({ taskId, targetAgentId, chatId: chatIdStr, messageLen: text.length }, 'Dispatched dashboard chat to agent process');
 ...
 */
export async function processMessageFromDashboard(botApi, text) {
    if (!ALLOWED_CHAT_ID)
        return;
    const chatIdStr = ALLOWED_CHAT_ID;
...
async function processDashboardMessage(botApi, text, chatIdStr) {
    ...
    try {
        ...
        const dashParts = [];
        if (agentSystemPrompt && !sessionId)
            dashParts.push(`[Agent role — follow these instructions]\n${agentSystemPrompt}\n[End agent role]`);
        if (memCtx)
            dashParts.push(memCtx);
        ...
        const result = await runAgent(fullMessage, sessionId, () => { }, // no typing action for dashboard
        onProgress, agentDefaultModel, abortCtrl, undefined, // no streaming for dashboard
        agentMcpAllowlist);
 ...
 */
export async function notifyWhatsAppIncoming(api, contactName, isGroup, groupName) {
    if (!ALLOWED_CHAT_ID)
        return;
