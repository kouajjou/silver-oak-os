...
let onHighImportanceMemory = null;
export function setHighImportanceCallback(cb) {
    ...
    try {
        ...
        if (embedding.length > 0) {
            ...
            for (const mem of existing) {
                ...
                if (sim > 0.85) {
                    logger.debug({ similarity: sim.toFixed(3), existingId: mem.id, newSummary: result.summary.slice(0, 60) }, 'Skipping duplicate memory');
                    return false;
                }
            }
        }
        const memoryId = saveStructuredMemoryAtomic(chatId, userMessage, result.summary, result.entities ?? [], result.topics ?? [], importance, embedding, 'conversation', agentId);
        // Notify on high-importance memories so the user can pin them
        if (importance >= 0.8 && onHighImportanceMemory) {
            try {
                onHighImportanceMemory(memoryId, result.summary, importance);
            }
            catch { /* non-fatal */ }
        }
        logger.info({ chatId, importance, memoryId, topics: result.topics, summary: result.summary.slice(0, 80) }, 'Memory ingested');
        return true;
    }
    catch (err) {
