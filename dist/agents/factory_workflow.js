...
// ── SaaS file helpers ─────────────────────────────────────────────────────
function createSaasDirectory(saasId) {
...
function updateSaasFile(saasPath, file, content) {
    const filePath = path.join(saasPath, file);
    fs.writeFileSync(filePath, content, 'utf-8');
}
function appendDecision(saasPath, decision) {
...
function buildStepPrompt(step, saasName, idea, previousOutput, testMode) {
...
// ── Main workflow ─────────────────────────────────────────────────────────
export async function runFactoryWorkflow(input) {
    ...
    // Announce start
    await sendTelegram(`🏭 *Factory SaaS démarrée*\n\n` +
        `💡 Idée: ${idea}\n` +
        `🆔 ID: \`${saasId}\`\n` +
    ...
    for (const step of FACTORY_STEPS) {
        ...
        if (step.hitl && !testMode) {
            await sendTelegram(`⏸️ *HITL Checkpoint — Étape ${step.step}: ${step.name}*\n\n` +
                `🏭 SaaS: ${saasName}\n` +
        ...
        try {
            ...
            if (step.step % 3 === 0) {
                await sendTelegram(`📊 *Factory Progress*\n` +
            ...
            await sendTelegram(`❌ *Factory Error — Étape ${step.step}: ${step.name}*\n\n` +
    ...
    // All steps complete
    const successMsg = `🎉 *Factory SaaS terminée!*\n\n` +
        `✅ ${saasName}\n` +
...
export function isFactoryRequest(message) {
    return FACTORY_PATTERNS.some((p) => p.test(message));
}
//# sourceMappingURL=factory_workflow.js.map