...
// ── Helpers ───────────────────────────────────────────────────────────────
async function sendTelegramProgress(text) {
...
async function dispatchToEmployee(employee, task, userId) {
...
export function classifyDomainRouteRegex(message) {
    ...
    try {
        ...
        for (const entry of dynamicRoutes) {
            ...
            if (DOMAIN_ROUTES.some(r => r.agentId === entry.agentId))
                continue;
            let score = 0;
            for (const pattern of entry.patterns) {
                const globalPattern = new RegExp(pattern.source, pattern.flags.includes('g') ? pattern.flags : pattern.flags + 'g');
            ...
            if (score > 0 && (!bestRoute || score > bestRoute.score)) {
                ...
                bestRoute = { intent: 'ops_task', agentId: entry.agentId, score };
            }
        }
    }
    catch {
...
async function classifyDomainRouteDynamic(message) {
    let agents;
    try {
...
Ta réponse (un seul mot, en minuscules) :`;
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('dynamic routing timeout')), DYNAMIC_ROUTING_TIMEOUT_MS));
        ...
        const intent = KNOWN_DOMAIN_INTENTS[matched.id] ?? 'research_task';
...
export async function classifyDomainRoute(message) {
...
export async function alexHandle(request) {
    const start = Date.now();
    ...
    try {
        ...
        if (regexBreakdown && regexBreakdown.length >= 2) {
            logger.info({ count: regexBreakdown.length, agents: regexBreakdown.map((t) => t.agent_target) }, '[ALEX] Multi-task pattern detected, routing to alexHandleAutonomous');
            try {
        ...
        if (intent.intent === 'technical_task' && intent.confidence > 0.6) {
            logger.info({ task: request.message.slice(0, 80), user: request.user_id }, '[ALEX] Delegating to Maestro for tech task');
            try {
                const maestroResult = await maestroHandle(request.message, {
                    userId: request.user_id,
                });
                logger.info({ success: maestroResult.success, mode: maestroResult.mode, taskId: maestroResult.taskId }, 'alex.delegated_to_maestro');
                return {
        ...
        if (domainRoute) {
            ...
            if (domainRoute.agentId === 'maestro') {
                logger.info({ msg: request.message.slice(0, 60) }, '[ALEX] Routing to Maestro via maestroHandle (Mode 1/2/3 dispatcher)');
                try {
            ...
            logger.info({ agentId: domainRoute.agentId, intent: domainRoute.intent, msg: request.message.slice(0, 60) }, '[ALEX] Domain routing to employee agent');
            try {
        ...
        if (process.env['USE_ALEX_PRO_MAX'] === 'true') {
            ...
            logger.info({ cost: 0, model: tmuxResult.model, mode: 'mode_1_tmux' }, 'alex.direct_reply.mode1');
            return {
                success: true,
...
export async function alexHandleAutonomous(request) {
    const start = Date.now();
    logger.info({ user: request.user_id, has_file: !!request.file_content, msg_len: request.message.length }, 'alex.autonomous.start');
    ...
    let failedCount = 0;
    for (let taskIndex = 0; taskIndex < Math.min(tasks.length, MAX_ITERATIONS); taskIndex++) {
        ...
        while (attempts < MAX_RETRIES_PER_TASK) {
            attempts++;
            logger.info({ task: task.id, attempt: attempts, target: task.agent_target }, 'alex.autonomous.dispatch');
            ...
            logger.info({ task: task.id, attempt: attempts, grade: judgeGrade, is_completed: isCompleted }, 'alex.autonomous.judged');
        ...
        if ((taskIndex + 1) % 5 === 0) {
            const progressMsg = `🔄 Alex Loop Progress — ${taskIndex + 1}/${tasks.length} taches traitees\n` +
    ...
    // 4. End run in DB
    const finalStatus = completedCount === tasks.length ? 'completed' : 'failed';
    if (runId) {
        await endRun(runId, finalStatus);
    }
    // 5. Build summary response
    const summaryLines = results.map((r, i) => `${i + 1}. [${r.grade}] ${r.task.title} → ${r.task.agent_target} (${r.attempts} essai${r.attempts > 1 ? 's' : ''})`);
    const summary = `🎯 Alex Autonomous Run terminé.\n\n` +
        `Tasks: ${tasks.length} total | ${completedCount} ✅ | ${failedCount} ❌\n` +
        `Cout: $${totalCost.toFixed(4)} | Run: ${runId ?? 'no-db'}\n\n` +
        summaryLines.join('\n');
    logger.info({ run_id: runId, completed: completedCount, failed: failedCount, cost: totalCost }, 'alex.autonomous.done');
    return {
...
//# sourceMappingURL=alex_orchestrator.js.map