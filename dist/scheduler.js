...
import { ALLOWED_CHAT_ID, MISSION_TIMEOUT_MS, agentDefaultModel, agentMcpAllowlist, agentSystemPrompt, } from './config.js';
import { getDueTasks, getSession, setSession, logConversationTurn, markTaskRunning, updateTaskAfterRun, resetStuckTasks, claimNextMissionTask, completeMissionTask, resetStuckMissionTasks, } from './db.js';
...
export function initScheduler(send, agentId = 'main') {
    if (!ALLOWED_CHAT_ID) {
...
function scheduleNextMissionPoll(delay) {
    setTimeout(async () => {
        try {
...
async function runDueMissionTasks() {
    ...
    logger.info({ missionId: mission.id, title: mission.title, type: mission.type, chatId: mission.chat_id }, 'Running mission task');
    messageQueue.enqueue(chatScope, async () => {
        ...
        try {
            ...
            const result = await runAgent(fullPrompt, sessionId, () => { }, undefined, agentDefaultModel, abortController, undefined, agentMcpAllowlist);
            clearTimeout(timeout);
            if (result.aborted) {
