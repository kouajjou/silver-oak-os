...
describe('schedule-cli agent routing', () => {
    ...
    it('auto-detects agent from CLAUDECLAW_AGENT_ID env var', () => {
        const result = createAndTrack(`node "${CLI_PATH}" create "test auto-detect" "0 9 * * *"`, cliEnv({ CLAUDECLAW_AGENT_ID: 'comms' }));
        expect(result).toContain('Agent:        comms');
    });
    it('--agent flag overrides CLAUDECLAW_AGENT_ID env var', () => {
        const result = createAndTrack(`node "${CLI_PATH}" create "test override" "0 9 * * *" --agent ops`, cliEnv({ CLAUDECLAW_AGENT_ID: 'comms' }));
        expect(result).toContain('Agent:        ops');
    });
    it('defaults to main when no env var and no --agent flag', () => {
        const result = createAndTrack(`node "${CLI_PATH}" create "test default" "0 9 * * *"`, cliEnv({ CLAUDECLAW_AGENT_ID: undefined }));
    ...
    function createAndTrack(cmd, env) {
...
//# sourceMappingURL=schedule-cli.test.js.map