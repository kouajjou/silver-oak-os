...
import { initSkillRegistry, getSkillIndex, matchSkills, getSkillInstructions, getAllSkills, } from './skill-registry.js';
// ── Helpers ─────────────────────────────────────────────────────────
...
describe('initSkillRegistry', () => {
    it('finds skills in a scanned directory', () => {
        writeSkill(path.join(tempRoot, 'skills'), 'gmail', `---
name: Gmail Manager
...
Read and send emails.`);
        ...
        writeSkill(path.join(tempGlobal, '.claude', 'skills'), 'gmail', `---
name: Gmail Manager
description: Manage Gmail inbox
triggers: email, inbox, gmail
---
...
describe('frontmatter parsing', () => {
    it('parses YAML frontmatter correctly', () => {
        writeSkill(path.join(tempGlobal, '.claude', 'skills'), 'calendar', `---
...
Create and manage events.`);
    ...
    it('falls back to H1 and first paragraph when no frontmatter', () => {
        writeSkill(path.join(tempGlobal, '.claude', 'skills'), 'todo', `# Task Manager
...
Run /todo to see tasks.`);
    ...
    it('falls back to directory name when no H1 or frontmatter', () => {
        writeSkill(path.join(tempGlobal, '.claude', 'skills'), 'my-skill', 'Just some plain text content here.');
        initSkillRegistry();
