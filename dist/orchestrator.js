 ...
 */
let cachedResponsibilityMap = null;
function loadSharedResponsibilityMap() {
    if (cachedResponsibilityMap !== null)
...
export function initOrchestrator() {
    ...
    logger.info({ agents: agentRegistry.map((a) => a.id) }, 'Orchestrator initialized');
}
 ...
 */
export async function delegateToAgent(agentId, prompt, chatId, fromAgent, onProgress, timeoutMs = DEFAULT_TIMEOUT_MS) {
    ...
    try {
        ...
        if (sharedMap) {
            contextParts.push(`[Shared responsibility map — AGENTS.md]\n${sharedMap}\n[End shared map]`);
        }
        if (systemPrompt) {
        ...
        try {
            const result = await runAgent(fullPrompt, undefined, // fresh session for each delegation
            () => { }, // no typing indicator needed for sub-delegation
            undefined, // no progress callback for inner agent
            undefined, // use default model
            abortCtrl, undefined, // no streaming for delegation
            ...
            onProgress?.(`${agent.name} completed (${Math.round(durationMs / 1000)}s)`);
            return {
                agentId,
                text: result.text,
                usage: result.usage,
