...
import type { MaestroTask } from './maestro_dispatcher.js';
...
let MAESTRO_SOP = '';
try {
  MAESTRO_SOP = fs.readFileSync(
    path.join(process.cwd(), 'agents/maestro/CLAUDE.md'),
    'utf-8'
  );
} catch (e) {
...
async function breakDownTaskSafe(task: string): Promise<string[]> {
  try {
    const tb: any = await import('./task_breaker.js').catch(() => null);
    if (tb && typeof tb.breakDownTasks === 'function') {
      const result = await tb.breakDownTasks(task, 'maestro-orchestrator');
      if (result && Array.isArray(result.tasks)) {
        return result.tasks.map((t: any) => t.description || t.title || String(t));
      }
    }
  } catch (e) {
...
async function judgeResultSafe(task: string, result: string): Promise<number | undefined> {
  try {
    const lj: any = await import('../services/llm_judge.js').catch(() => null);
    if (lj && typeof lj.judge === 'function') {
      ...
      if (judged && judged.grade) {
        const gradeMap: Record<string, number> = { A: 1.0, B: 0.8, C: 0.6, D: 0.4, F: 0.0 };
...
export async function maestroHandle(
  task: string,
  context: MaestroOrchestratorContext = {}
): Promise<MaestroOrchestratorResult> {
  const startTime = Date.now();
  const taskId = 'maestro_' + startTime + '_' + Math.random().toString(36).slice(2, 8);

  logger.info(
    { taskId, len: task.length, context },
    '[MAESTRO_ORCH] received task'
  );
  ...
  try {
    ...
    logger.info(
      { mode: classification.mode, confidence: classification.confidence, reason: classification.reasoning },
      '[MAESTRO_ORCH] mode selected'
    );

    // 3. Build MaestroTask for dispatcher
    const dispatchPayload: MaestroTask = {
      task_description: task,
    ...
    if (context.preferredProvider && context.preferredProvider !== 'gemini' && context.preferredProvider !== 'grok' && context.preferredProvider !== 'mistral') {
      dispatchPayload.preferred_provider = context.preferredProvider as any;
    }
    ...
    logger.info(
      { success: dispatchResult.success, provider: dispatchResult.provider_used, mode: dispatchResult.mode_used },
      '[MAESTRO_ORCH] dispatch result'
    );
