...
function extractAutonomy(titleLine: string): AutonomyLevel {
  if (titleLine.includes('[AUTO]')) return 'AUTO';
  if (titleLine.includes('[NOTIFY]')) return 'NOTIFY';
  if (titleLine.includes('[HITL]')) return 'HITL';
  return 'NOTIFY'; // safe default
}

function extractTaskId(titleLine: string): string {
  const m = titleLine.match(/###\s+(T\d+)/);
  return m ? m[1] : 'T?';
}

function extractTitle(titleLine: string): string {
  return titleLine
...
function generateBranch(taskId: string, title: string): string {
  const slug = title
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
 ...
 */
function parseDependencies(depSection: string, tasks: BacklogTask[]): void {
  ...
  for (const line of lines) {
    ...
    if (!waitKeywords.test(line)) continue;
 ...
 */
export function parseBacklog(content: string): TaskWave {
  ...
  const metadata: BacklogMetadata = {
    ...
    hitlReactivity: (hitlMatch?.[1]?.toLowerCase() as BacklogMetadata['hitlReactivity']) ?? 'normale',
  ...
  const taskStarts: { index: number; line: string }[] = [];
 ...
 */
export function getReadyTasks(wave: TaskWave): BacklogTask[] {
 ...
 */
export function applyAntiChurn(task: BacklogTask): boolean {
  task.failCount++;
  if (task.failCount >= 2) {
    task.status = 'skipped';
    return true;
