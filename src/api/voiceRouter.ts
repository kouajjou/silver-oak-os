...
export function startVoiceApiServer(): void {
  ...
  app.post('/api/voice/chat/:agentId', async (c) => {
    ...
    try {
      ...
      const parseTmuxReply = (rawContent: string): { reply: string; isEchoOnly: boolean } => {
        let raw = rawContent;
        ...
        const rawContainsMarker =
          /FIN DE T[ÂA]CHE OBLIGATOIRE/i.test(raw) ||
        ...
        const isEchoOnly =
          (rawContainsMarker && cleaned.length < 10) ||
          cleaned.length < 3;
      ...
      if (useVoiceProMax) {
        ...
        const MAX_RETRIES = 2;
        let parsed: { reply: string; isEchoOnly: boolean } | null = null;
        let lastLatency = 0;

        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          logger.info(
            { agentId, session: 'claude-frontend', attempt },
            'voice.chat.mode1_tmux'
          );
          ...
          logger.warn(
            { agentId, attempt, replyPreview: parsed.reply.slice(0, 100) },
            'voice.parser.echo_only_retry'
          );
        }
        ...
        if (parsed && !parsed.isEchoOnly) {
          reply = parsed.reply;
          logger.info(
            { agentId, latency: lastLatency, cost: 0 },
          ...
          logger.warn(
            { agentId },
            'voice.chat.mode1_failed_fallback_gemini'
          );
          if (!geminiKey) {
