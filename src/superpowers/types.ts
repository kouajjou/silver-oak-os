...
export interface SuperPowersRequest extends LLMRequest {
  ...
  tools?: string[];        // names of tools to enable from registry
  max_iterations?: number; // for future tool execution loops
}

export interface SuperPowersResponse extends LLMResponse {
  tool_calls_executed: Array<{ name: string; input: Record<string, unknown>; output: Record<string, unknown> }>;
  iterations: number;
