...
function readTrait(traitName: string): string {
  const filePath = path.join(SOUL_PROMPTS_DIR, 'traits', `${traitName}.md`);
  return readBlock(filePath);
}

function readLanguageBlock(lang: 'fr' | 'en' | 'es'): string {
  const filePath = path.join(SOUL_PROMPTS_DIR, 'languages', `${lang}.md`);
  return readBlock(filePath);
}

function readRoleBlock(role: 'orchestrator' | 'specialist' | 'workhorse'): string {
  const filePath = path.join(SOUL_PROMPTS_DIR, 'roles', `${role}.md`);
  return readBlock(filePath);
}

function readSharedBlock(blockName: string): string {
...
function getLangLabel(lang: 'fr' | 'en' | 'es'): string {
...
export function buildSoulPrompt(spec: SoulPromptSpec): string {
  const {
    ...
    customMission,
    delegationRules,
    sharedBlocks = DEFAULT_SHARED_BLOCKS,
  } = spec;
