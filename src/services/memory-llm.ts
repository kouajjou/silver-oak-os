...
async function tryMistral(prompt: string, context: string): Promise<string | null> {
  ...
  try {
    ...
    logger.info(
      { provider: 'mistral', model: res.model, cost_usd: res.cost_usd, latency_ms: res.latency_ms },
      '[memory-llm] call',
    );
    return res.content;
  } catch (err) {
...
async function tryGeminiFallback(prompt: string, context: string): Promise<string | null> {
  try {
    const res = await callLLM({
      provider: 'google',
    ...
    logger.info(
      { provider: 'gemini-fallback', model: res.model, cost_usd: res.cost_usd, latency_ms: res.latency_ms },
      '[memory-llm] call',
    );
    return res.content;
  } catch (err) {
 ...
 */
export async function callMemoryLLM(prompt: string, context = ''): Promise<string> {
  ...
  throw new MemoryLLMError(
    `Both Mistral and Gemini fallback failed for memory call (latency ${totalLatency}ms)`,
  );
}
