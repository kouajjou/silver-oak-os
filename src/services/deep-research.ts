...
async function callWithFallback(
  ...
  query: string,
): Promise<{ content: string; model: string; latency_ms: number; cost_usd: number } | null> {
  const messages = [
    { role: 'system' as const, content: 'Tu es un chercheur. Réponds avec des faits vérifiables, des sources si possible, et une date de fraîcheur.' },
 ...
 */
export async function deepResearch(query: string): Promise<ResearchReport> {
