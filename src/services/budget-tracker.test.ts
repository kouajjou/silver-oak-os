...
describe('trackCost — agent_id validation', () => {
  it('accepts canonical agent ids: main/comms/content/maestro/ops/research', () => {
    ...
    const rows = getMemDb().prepare('SELECT COUNT(*) as c FROM agent_costs').get() as { c: number };
  ...
  it('rejects empty / missing / non-string agent_id without inserting', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    trackCost({ ...baseEntry, agent_id: '' as any });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    trackCost({ ...baseEntry, agent_id: null as any });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    trackCost({ ...baseEntry, agent_id: 42 as any });
  ...
  it('rejects blocked dev/test artefact patterns', () => {
    ...
    for (const id of blocked) trackCost({ ...baseEntry, agent_id: id });
    const rows = getMemDb().prepare('SELECT COUNT(*) as c FROM agent_costs').get() as { c: number };
    expect(rows.c).toBe(0);
  });
  ...
  it('rejects unknown agent ids that match no whitelist pattern', () => {
    ...
    for (const id of unknowns) trackCost({ ...baseEntry, agent_id: id });
    const rows = getMemDb().prepare('SELECT COUNT(*) as c FROM agent_costs').get() as { c: number };
    expect(rows.c).toBe(0);
  });
});
...
describe('cleanupBudgetArtefacts', () => {
  it('removes rows matching blocked dev patterns and reports counts + samples', () => {
    ...
    const stmt = db.prepare(
      'INSERT INTO agent_costs (agent_id, cost_usd, tokens_in, tokens_out, model) VALUES (?, 0.01, 0, 0, ?)'
    );
    stmt.run('test-fix', 'm');
    ...
    const remaining = db.prepare('SELECT agent_id FROM agent_costs ORDER BY agent_id').all() as Array<{ agent_id: string }>;
