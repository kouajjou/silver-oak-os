...
// Helpers
function mockSystemctlState(agentId: string, opts: { active?: boolean; nRestarts?: number; loaded?: boolean }) {
...
describe('crash loop detection (via timer simulation)', () => {
  it('detects crash loop after threshold restarts in window and triggers Telegram alert', async () => {
    ...
    const telegramCalls = mockedFetch.mock.calls.filter(
      (c) => typeof c[0] === 'string' && (c[0] as string).includes('telegram.org')
    );
  ...
  it('skips silently if systemctl returns "LoadState=not-found" (agent not loaded)', async () => {
    ...
    // No Telegram, no stop
    const tgCalls = mockedFetch.mock.calls.filter(
      (c) => typeof c[0] === 'string' && (c[0] as string).includes('telegram.org')
    );
